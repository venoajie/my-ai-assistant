...
class WriteFileTool(Tool):
    ...
    def __call__(self, path: str, content: str) -> Tuple[bool, str]:
        p = Path(path)
        try:
...
class RunShellCommandTool(Tool):
    ...
    def __call__(self, command: str) -> Tuple[bool, str]:
        # --- 1. Input Validation ---
        ...
        if len(command) > 2000:  # Increased limit slightly for flexibility
            return (False, "🚫 SECURITY: Command exceeds maximum length (2000 characters).")
        ...
        # --- 3. Additional Suspicious Pattern Detection ---
        suspicious_patterns = [
            r'\|.*sh\b',      # Piping to shell
            r'eval\s*\(',     # eval() calls
            r'exec\s*\(',     # exec() calls  
            r'__import__',    # Python imports via shell
            r'subprocess',    # Python subprocess calls via shell
        ]
        
        for pattern in suspicious_patterns:
            if re.search(pattern, command, re.IGNORECASE):
                return (False, f"🚫 SECURITY BLOCK: Command contains suspicious pattern '{pattern}'.")
        
        # --- 4. Environment Variable Protection ---
        protected_vars = ['PATH', 'HOME', 'USER', 'SHELL', 'LD_LIBRARY_PATH']
        for var in protected_vars:
            if f'unset {var}' in command.lower() or f'export {var}=' in command.lower():
                return (False, f"🚫 SECURITY BLOCK: Attempt to modify protected environment variable '{var}'.")
        
        try:
            # --- 5. Secure Execution with Explicit CWD ---
            result = subprocess.run(
                ...
                check=False, 
                timeout=60,
                cwd=Path.cwd()  # Explicitly set current working directory
            )
            
            # --- 6. Output Sanitization (Truncation) ---
...
class GitAddTool(Tool):
    ...
    def __call__(self, path: str, force: bool = False) -> Tuple[bool, str]:
        command = ["git", "add"];
...
class GitCommitTool(Tool):
    ...
    def __call__(self, commit_message: str) -> Tuple[bool, str]:
...
class GitCheckoutTool(Tool):
    ...
    is_risky = True # Risky because it can discard uncommitted changes in other files
    def __call__(self, branch_name: str) -> Tuple[bool, str]:
